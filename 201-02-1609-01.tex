...
\begin{solution}
	...
	Once again, we see that \( d^{(3)} \le 0 \iff d  \le 0 \), which is never the case, thus it is non-negative and also \( d^{(3)}(x,y) = 0 \iff d(x,y) = 0 \iff x = y  \). Also the symmetry of \( d \) implies the symmetry of \( d^{(3)} \). For the triangle inequality
	\begin{align*}
...
\begin{solution}
	Assume for the sake of contradiction that \( d \) is not continuous i.e.
	\\
	Let \( x_1, x_{2} \in X\), then
	\[
		\exists \ \epsilon > 0 \text{ such that } \ \forall \ \delta > 0 \ 0 < |x_{1}-x_{2}| < \delta \Rightarrow |d(x_1,x_2)| \ge \epsilon
	\]
	Aim: to show that \( (X \times X, d_1) \) is not a metric space under the given assumptions

	Clearly \( d_1 \) is non-negative since the sum of two non-neg is non-neg

	Also \( d_1 = 0 \iff x_{1} = x_{3} \text{ and } x_{2} = x_{4} \) implying \( (x_{1},x_{2}) = (x_{3}, x_{4}) \)

	Clearly \( d_1 \) is symmetric

	Triangle Inequality
	\begin{align*}
		d_{1}((x_{1},x_{2}),(x_{3},x_{4})) & = d(p_{1}) + d(p_{2})                                     \\
		                                   & \le d(x_{1},y) + d(y,x_{3}) + d(x_{2},z) + d(z,x_{4})     \\
		                                   & \le d_1((x_{1},x_{2}),(y,z)) + d_{1}((y,z),(x_{3},x_{4}))
	\end{align*}
...
Define
\[
	d(x,y) = \sum_{n=1}^{\infty} \frac{1}{2^j}\frac{|x_j-y_j|}{1+|x_j-y_j|}
\]
